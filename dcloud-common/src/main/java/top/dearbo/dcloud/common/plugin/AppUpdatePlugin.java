...
import top.dearbo.common.appupdate.model.UpdateBean;
...
import top.dearbo.common.util.ToastUtil;
import top.dearbo.common.view.ConfirmDialog;
            ...
            if (!EasyPermissions.hasPermissions(iWebview.getActivity(), permissionsList)) {
                PermissionUtils.permission(permissionsList).callback(new PermissionUtils.SimpleCallback() {
                    ...
                    public void onDenied() {
                        JSUtils.execCallbackOk(iWebview, callbackId, new AjaxResult(-1, "权限请求失败!"));
                    }
                }).request();
            }
        }
